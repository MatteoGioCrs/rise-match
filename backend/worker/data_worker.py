...
# Codes événements SwimCloud → codes internes RISE
SWIMCLOUD_EVENTS = {
    '50FR':   '1|50|1',
    '100FR':  '1|100|1',
    '200FR':  '1|200|1',
...
CURRENT_YEAR = 2026  # saison 2025-26
...
async def fetch_top_times(client: httpx.AsyncClient, event_code: str, region: str, page: int) -> dict:
...
async def main():
    ...
    teams: dict = {}    # swimcloud_id → team dict
    swimmers: dict = {} # swimcloud_id → swimmer dict (avec times)
    ...
    async with httpx.AsyncClient(headers=headers) as client:
        for division in DIVISIONS:
            for event_code in SWIMCLOUD_EVENTS.keys():
                ...
                while True:
                    try:
                        data = await fetch_top_times(client, event_code, division, page)
                        ...
                        for splash in results:
                            ...
                            # Meilleur temps (premier = meilleur car trié par l'API)
                        ...
                        print(f"  {division} {event_code} p{page}/{page_count} → {len(results)}")
                        ...
                        print(f"  ERREUR {division} {event_code} p{page}: {e}")
                        break
    ...
    for sw in swimmers.values():
        ...
        await conn.execute("""
            INSERT INTO sc_swimmers (swimcloud_id, name, gradcollege, dateofbirth, country,
                                     team_swimcloud_id, study_year, is_departing, updated_at)
            VALUES ($1,$2,$3,$4,$5,$6,$7,$8,now())
            ON CONFLICT (swimcloud_id) DO UPDATE
            SET name=$2, gradcollege=$3, dateofbirth=$4, country=$5,
                team_swimcloud_id=$6, study_year=$7, is_departing=$8, updated_at=now()
        """, sw['swimcloud_id'], sw['name'], gc, sw['dateofbirth'],
             sw['country'], sw['team_swimcloud_id'],
             get_study_year(gc), is_departing(gc))
